...
def plm(index, word_prior, best_doc_indices, query_word_ids, k, mu):
    scores_per_doc = {}
...
for query_id, query in topics.items():
    ...
    if len(query_indices) != 0:
        ...
        scores = np.zeros(len(best_1000_doc_indices))
        results[query_id] = list(zip(scores, best_1000_doc_indices))  # use previous scoring...
model_name = 'plm' + kernel_name
utils.write_run(model_name=model_name, data=results,
                out_f='results/ranking_' + model_name + '_' + str(mu) + '.txt', max_objects_per_query=1000)
